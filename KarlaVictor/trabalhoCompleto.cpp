...
int main(){
    ...
    imprimeMatriz( coluna, linha);//1
    vetorzao(coluna, linha);//2
    maiorLinha(coluna, linha);
    menorLinha(coluna, linha);
    menorColuna(coluna,linha);
    maiorColuna(coluna,linha);//3
    diagonal(coluna, linha);
    diagonal2(coluna, linha);//4
    ...
    cout<<"informe a coluna para inverter(0 a 99): ";
    cin>> nColuna;
    inverterColuna(coluna, linha, nColuna);//5
    char **matrizNome;
    matrizNome = new char*[linha];
    for(int i =0;i<linha;i++){
        matrizNome[i] = new char[coluna];
    }
    preencherNome(matrizNome,linha,coluna);//6
